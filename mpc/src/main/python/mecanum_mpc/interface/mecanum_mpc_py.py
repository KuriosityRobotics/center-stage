...
a Python wrapper for a fast solver generated by FORCESPRO v6.3.0
   ...
   OUTPUT = mecanum_mpc_py.mecanum_mpc_solve(PARAMS) solves a multistage problem
   ...
   [OUTPUT, EXITFLAG] = mecanum_mpc_py.mecanum_mpc_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = mecanum_mpc_py.mecanum_mpc_solve(PARAMS) returns 
...
try:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'mecanum_mpc/lib/mecanum_mpc_withModel.so'))
    csolver = getattr(_lib,'mecanum_mpc_solve')
except:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'mecanum_mpc/lib/libmecanum_mpc_withModel.so'))
    csolver = getattr(_lib,'mecanum_mpc_solve')

internal_mem = getattr(_lib,'mecanum_mpc_internal_mem')

class mecanum_mpc_params_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('x0', ctypes.c_double * 50),
('xinit', ctypes.c_double * 6),
('all_parameters', ctypes.c_double * 160),
]

mecanum_mpc_params = {'x0' : np.array([]),
'xinit' : np.array([]),
...
mecanum_mpc_params_types = {'x0' : np.float64,
'xinit' : np.float64,
'all_parameters' : np.float64,
}

class mecanum_mpc_outputs_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
mecanum_mpc_outputs = {'x1' : np.array([]),
'x2' : np.array([]),
'x3' : np.array([]),
'x4' : np.array([]),
'x5' : np.array([]),
...
class mecanum_mpc_info(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
class mecanum_mpc_mem(ctypes.Structure):
    pass
...
csolver.argtypes = ( ctypes.POINTER(mecanum_mpc_params_ctypes), ctypes.POINTER(mecanum_mpc_outputs_ctypes), ctypes.POINTER(mecanum_mpc_info), ctypes.POINTER(mecanum_mpc_mem), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

internal_mem.argtypes = [ctypes.c_uint]
internal_mem.restype = ctypes.POINTER(mecanum_mpc_mem)

def mecanum_mpc_solve(params_arg):
    '''
a Python wrapper for a fast solver generated by FORCESPRO v6.3.0
 ...
 See also COPYING
    ...
    # convert parameters
    params_py = mecanum_mpc_params_ctypes()
    for par in params_arg:
        ...
        try:
            ...
            if isinstance(getattr(params_py, par), ctypes.Array):
                params_arg[par] = np.require(params_arg[par], dtype=mecanum_mpc_params_types[par], requirements='F')
    ...
    outputs_py = mecanum_mpc_outputs_ctypes()
    info_py = mecanum_mpc_info()
    if sys.version_info.major == 2:
        ...
        try:
            PyFile_AsFile.restype = ctypes.POINTER(FILE)
            exitflag = _lib.mecanum_mpc_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), PyFile_AsFile(fp) , _lib.mecanum_mpc_adtool2forces )
        ...
        try:
            if sys.platform.startswith('win'):
                exitflag = _lib.mecanum_mpc_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), None , _lib.mecanum_mpc_adtool2forces)
            else:
                exitflag = _lib.mecanum_mpc_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), fp , _lib.mecanum_mpc_adtool2forces)
            libc.fclose(fp)
    ...
    # convert outputs
    for out in mecanum_mpc_outputs:
        mecanum_mpc_outputs[out] = npct.as_array(getattr(outputs_py,out))

    return mecanum_mpc_outputs,int(exitflag),info_py

solve = mecanum_mpc_solve

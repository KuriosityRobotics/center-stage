...
a Python wrapper for a fast solver generated by FORCESPRO v6.0.1
   ...
   subject to the parameters supplied in the following dictionary:
       ...
       PARAMS['all_parameters'] - matrix of size [310x1]
...
_lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'mecanum_mpc/lib/libmecanum_mpc.so'))
...
#    def from_param(self):
#        return self
    _fields_ = [('x0', ctypes.c_double * 100),
('xinit', ctypes.c_double * 6),
('all_parameters', ctypes.c_double * 310),
]
...
mecanum_mpc_outputs = {'x01' : np.array([]),
'x02' : np.array([]),
'x03' : np.array([]),
'x04' : np.array([]),
'x05' : np.array([]),

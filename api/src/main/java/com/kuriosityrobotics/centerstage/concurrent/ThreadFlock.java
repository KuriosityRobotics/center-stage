...
import static com.kuriosityrobotics.centerstage.concurrent.Util.toIdentityString;
...
public class ThreadFlock implements AutoCloseable {
	...
	private final ThreadFlock.ThreadContainerImpl container; // encapsulate for now
	...
	private final AtomicBoolean permit = new AtomicBoolean();

	ThreadFlock(String name) {
		this.name = name;
		this.container = new ThreadFlock.ThreadContainerImpl(this);
	}
	...
	private void decrementThreadCount() {
		int count = (int) threadCount.getAndAdd(-1) - 1;
	...
	private boolean getAndSetPermit(boolean newValue) {
		return permit.getAndSet(newValue);
	}
	...
	public Thread start(Thread thread) {
		...
		if (initException.get() != null)
			System.out.println("ERROR in thread flock: " + initException.get());
		return thread;
	}
	...
	public boolean awaitAll() throws InterruptedException {
		...
		while (threadCount.get() > 0 && !permit.get()) {
			LockSupport.park();
	...
	public boolean awaitAll(Duration timeout)
		...
		while (threadCount.get() > 0 && remainingNanos > 0 && !permit.get()) {
		...
		boolean done = (threadCount.get() == 0);
		if (!done && remainingNanos <= 0 && !permit.get()) {
	 ...
	 *
	 * @throws WrongThreadException        if invoked by a thread that is not the owner
	 * @throws StructureViolationException if a structure
	...
	public String toString() {
		String id = toIdentityString(this);
		if (name != null) {
			return name + "/" + id;
		} else {
			return id;
	...
	private static class ThreadContainerImpl extends ThreadContainer {
		...
		@Override
		public ThreadFlock.ThreadContainerImpl push() {
		...
		void close() {
			...
			if (!closing) {
				...
				if (!atTop)
					throw new StructureViolationException("ThreadFlock.close called from within a nested flock");
			}
		}
